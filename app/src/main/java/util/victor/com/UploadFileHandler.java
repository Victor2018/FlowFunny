    ...
    private void processMultipartContentRequest(HttpEntityEnclosingRequest request) throws IOException{
        ...
        try {
            ...
            while(iter.hasNext()) {
                ...
                if(item.isFormField()) {
                    Log.d("zhouchuan", "Form field " + name + " with value " + Streams.asString(stream) + " detected.");
                }else {
                    Log.d("zhouchuan", "File field " + name + " with file name " + item.getName() + " detected.");
                    try {
                        ...
                        Log.d("zhouchuan", "-------processMultipartContentRequest------------------------f=" + f);
                        if(f.isDirectory()) {
//							OutputStream out = new FileOutputStream(filePath + fileName);
                            ...
                            Log.d("zhouchuan", "------------------fileSize=" + fileSize);
                            if(fileSize > 0) {
                                if (this.validateZipFile(file.getAbsolutePath())) {
                                    ...
                                    //让线程进入循环等待，等待安装结构然后给用户响应，如果等待时间达到上限则停止等待
                                    isInstall = true;
                                    int waitTime = 0;
                                    while (isInstall && waitTime < MaxWaitTime) {
                                        Log.d("zhouchuan", "----------姝ｅ湪瀹夎绋嶇瓑-----------");
                                        Thread.sleep(1000);
                                        waitTime++;
                                    }
                                    //达到等待最大时间则响应安装失败
